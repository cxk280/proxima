...
To boot fresh responders instead: `./deploy/probes-up.sh ewr sjc lhr nrt syd bom` then paste
its printed JSON into `app-deploy.sh`.

## Teardown (after the demo)

```bash
./deploy/probes-down.sh           # kills the 6-region fleet (tag proxima-probe only)
```
